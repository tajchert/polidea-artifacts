...
module Polidea::Artifacts
  class Processor
    ...
    def process_archive!(path, artifact_paths)
      ...
      File.open(installation_page_url, 'w') {|f| f.write(page_generator.generate_page_url(Pathname.new(@base_upload_url) + Pathname.new(upload_path) + manifest_path.basename))}
    ...
    def process_android_archive!(path, artifact_paths)
      ...
      installation_page_url = Pathname.new(tmp_dir) + 'install.html'
      File.open(installation_page_url, 'w') {|f| f.write(page_generator.generate_page_url(Pathname.new(@base_upload_url) + Pathname.new(upload_path) + apk_pathname.basename))}
      copy_artifact(installation_page_url, artifact_paths)
    end
